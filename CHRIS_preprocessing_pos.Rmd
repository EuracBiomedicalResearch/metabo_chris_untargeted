...
save(chris, file = "chris_after_peakdetect.RData")  #!!!FINDME, path
```

```{r, echo = FALSE, eval = file.exists("chris_after_peakdetect.RData")}
#' Loading the already peak-detected data
load("chris_after_peakdetect.RData")  #!!!FINDME path
```
...
check all the (internal) stardards, see list in general\_data_overview report.
load this file first.

```{r load standers list}
##' list 3: standards_chris_pos
#' load the good quality internal and in-house standards list, see general_data_overview
...
#standard <- read.delim(file.path(DATA_PATH, "Mix_alignment.txt"), comment.char = "#")
...
#' run with that first
#' Define parameters of choice  #https://rdrr.io/bioc/xcms/man/adjustRtime-peakGroups.html
#' #nok? #!!!FINDME TODO
#' #' error in function PeakGroupsParam, update all BiocManager packages first to devel version
#save <- final_table
#final_table <- final_table[1:3,]
...
col_batches <- col_batch[sampleData(chris)$batch_id]
pool_index <- which(sampleData(chris)$sample_type == "Pool")
batches_pool <- col_batch[sampleData(chris)$batch][pool_index]
...
```{r include=FALSE}
save(chris, file = "chris_after_alignment.RData")
```
...
```{r include=FALSE}
save(chris, file = "chris_after_correspondence.RData")
```
...
#' Plot the EICs
dr <- file.path(IMAGE_PATH, "EIC_standard/full/after_correspondence/")
dir.create(dr, recursive = TRUE, showWarnings = FALSE)
...
intern_standard_eic <- featureChromatograms(
    chris, features = intern_standard_match$target_feature_id,
    expandRt = 2, chunkSize = NR_CORES)
...
load("chris_after_gap_filling.RData")
```
...
save(chris, file = "chris_after_gap_filling.RData")
```
...
save(blank_sample, file = "blank_chris.RData")
save(chris, file = "samples_chris.RData")
```
...
filled = TRUE )
save(res_blank, file = "SumExp_chris_blank.RData")
```
