...
IMAGE_PATH <- paste0("images/", filename, "/", BATCHNR, "/")
if (file.exists(IMAGE_PATH))
    unlink(IMAGE_PATH, recursive = TRUE)
dir.create(IMAGE_PATH, recursive = TRUE)

#' Path to store RData files
RDATA_PATH <- paste0("data/RData/", filename, "/", BATCHNR, "/")
dir.create(RDATA_PATH, recursive = TRUE, showWarnings = FALSE)

#' Paths to the data
DATA_SPATH <- '/data/massspec/sqlite'
#DATA_PATH <- '/media/mdegraeve/josung/data/CHRIS'     #FINDME!
DATA_PATH <- "data"
...
handle <- file.path(DATA_SPATH, 'chris_hilic_pos_1.0.0.2.sqlite')
...
#chris <- chris[sampleData(chris)$sample_type %in%  "Pool"]

#' Filter for ret time
chris <- filterRt(chris, c(10, 250))

##' for testing subset chris
...
#' Select only one pool sample to see good new anchorpoints across batches,
#' code stops at violoin plots as no bio samples
#chris <- chris[grep("POOL_5", sampleData(chris)$sample_id)]

#' keep min 1 QC and min 3 biol samples per batch, total 5 samples per batch
#head(sampleData(chris))
...
print('sample_id:')
#unique(sampleData(chris)$sample_id)
#length(unique(sampleData(chris)$sample_id))

print('nr of samples in subset of chris:')
length(chris)
...
```{r ppm_parameter decide with example 1, eval = FALSE}
##' example 1
#' cystine
...
```{r check ppm ok with example 2, eval = FALSE}
##' example 2
#' histidine
...
```{r findchrompeaks_all_data, echo=TRUE, eval = !file.exists(paste0(RDATA_PATH, "chris_after_peakdetect.RData"))}
chris <- findChromPeaks(chris, param = param, chunkSize = NR_CORES)
save(chris, file = paste0(RDATA_PATH, "chris_after_peakdetect.RData"))

```

```{r load peakpick rdata, echo = FALSE, eval = file.exists(paste0(RDATA_PATH, "chris_after_peakdetect.RData"))}
...
```{r check_standards_dilution, echo = FALSE, eval = FALSE}
#' Create EICs for all standards and plot them to evaluate a) presence of the
#' ion in the current batch and b) check/adjust its retention time
standards_all <- read.delim(file.path(DATA_PATH, "standards_dilution.txt"),
                            sep = "\t")
standards_all <- standards_all[!is.na(standards_all$POS), ]
standards_all$mass <- calculateMass(standards_all$formula)
standards_all$mz <- mapply(standards_all$mass, standards_all$POS, FUN = mass2mz)

#' Extract EICs, using a 20ppm m/z range.
standards_all$mzmin <- standards_all$mz - MsCoreUtils::ppm(standards_all$mz, 10)
standards_all$mzmax <- standards_all$mz + MsCoreUtils::ppm(standards_all$mz, 10)
standards_all$rtmin <- standards_all$RT - 10
standards_all$rtmax <- standards_all$RT + 10
standards_all$rtmin[is.na(standards_all$rtmine)] <- -Inf
standards_all$rtmax[is.na(standards_all$rtmine)] <- Inf

chris_batch <- split(chris, sampleData(chris)$batch_id)

#' Iterate over batches to create EIC plots separate for each batch.
lapply(chris_batch, function(z) {

    #' Extract EICs, full rt range, and only restricted to expected RT.
    standards_all_eic_full <- chromatogram(
        z, mz = cbind(standards_all$mzmin, standards_all$mzmax),
        chunkSize = NR_CORES)
    standards_all_eic <- chromatogram(
        z, mz = cbind(standards_all$mzmin, standards_all$mzmax),
        rt = cbind(standards_all$rtmin, standards_all$rtmax),
        chunkSize = NR_CORES)

    dr <- file.path(IMAGE_PATH, "standards_all_eics", sampleData(z)$batch_id[1L])
    dir.create(dr, recursive = TRUE)

    for (i in seq_len(nrow(standards_all))) {
        fl <- paste0(i, "-", standards_all$name[i], ".png")
        png(file.path(dr, fl), width = 14, height = 7, units = "cm", res = 600,
            pointsize = 4)
        par(mfrow = c(1, 2))
        tmp <- standards_all_eic_full[i, ]
        plot(tmp,
             col = paste0(col_samples, 60),
             peakCol = col_samples[chromPeaks(tmp)[, "sample"]],
             peakBg = paste0(col_samples[chromPeaks(tmp)[, "sample"]], 10))
        grid()
        abline(v = standards_all$RT[i], col = "red", lty = 2)
        tmp <- standards_all_eic[i, ]
        plot(tmp, main = standards_all$name[i],
             col = paste0(col_samples, 60),
             peakCol = col_samples[chromPeaks(tmp)[, "sample"]],
             peakBg = paste0(col_samples[chromPeaks(tmp)[, "sample"]], 10))
        grid()
...
#' Based on these EICs the data/standards_chris_pos_batch.txt file should be
#' updated adding observed retention times for each batch as a new column.
#' This file might be used at a later stage for annotation of features.
...
```{r all eics pool after peak detection, include = TRUE, eval = FALSE}
#' All EICs in pool
...
for (i in seq_len(nrow(standards_chris_pos))) {
    nm <- standards_chris_pos$name[i]
    nm <- gsub("%", "perc", nm, fixed = TRUE)
    png(paste0(dr, "EIC_", nm, ".png"),
    ...
    grid()
    abline(v = standards_chris_pos$RT[i], col = "#ff000080", lty = 2)
...
format(round(median(table(chromPeaks(chris)[, "sample"])), 3), 3)
...
`r format(round(mean(table(chromPeaks(chris)[, "sample"])), 3), 3)`.
...
```{r find good qual peak for rt align in rt 50-150s range, eval = FALSE}
#' keep only 2 samples
mse <- chris[c(1L, 2L)]
#mse
#length(mse)
...
#add to mixalign if improves = NO
...
#' plot 3D centroided on this rt region
mse |>
    filterRt(rt = c(135, 145)) |> #10s
    filterMzRange(mz = c(702.489, 702.504)) |> #0.02 da
    plot()
...
eic_test <- chromatogram(chris,
                    ...
                    chunkSize = NR_CORES)
plot(eic_test,
     main = "EIC for test rt 140, mz 702")
...
#' plot 3D centroided on this rt region
mse |>
    filterRt(rt = c(115, 125)) |> #10s
    filterMzRange(mz = c(491.295, 491.305)) |> #0.02 da
    plot()
...
```{r refine_chrom_peaks,  message = FALSE, eval = !file.exists(paste0(RDATA_PATH, "chris_after_refine.RData"))}
...
```{r load_refine_chrom_peaks, include=FALSE, eval = file.exists(paste0(RDATA_PATH, "chris_after_refine.RData"))}
...
```{r all eics pool after peak refinement, include = FALSE, eval = FALSE}
#' All EICs in pool
eics <- chromatogram(chris[pool_index],
                     mz = as.matrix(standards_chris_pos[, c("mzmin", "mzmax")]),
                     rt = as.matrix(standards_chris_pos[, c("rtmin", "rtmax")]),
...
```{r chris_copy_for_evaluation}
#' keep a raw chris object to compare before/after alignment
chris_raw <- chris

```

```{r check new anchor points, eval = FALSE, echo = FALSE}
#' Check if we could find some signal (even if not from a standard) between
#' 70 and 150 seconds we could use for alignment.
tmp <- chris[pool_index]

#' Identify chrompeaks with a high signal in this region.
cpks <- chromPeaks(tmp, rt = c(70, 150))
cpks <- cpks[cpks[, "maxo"] > 10000, ]

#' Group the chrom peaks by m/z
mz_groups <- MsCoreUtils::group(cpks[, "mz"], ppm = 10)
cpks_groups <- split.data.frame(cpks, mz_groups)

#' Extract the median m/z and the rt range for all
tmp_pks <- lapply(
    cpks_groups, function(z)
        c(median(z[, "mz"]), min(z[, "rtmin"]), max(z[, "rtmax"]))) |>
    do.call(what = rbind)
tmp_pks[, 2] <- tmp_pks[, 2] - 10
tmp_pks[, 3] <- tmp_pks[, 3] + 10

#' extract EICs.
tmp_eics <- chromatogram(
    chris, mz = cbind(tmp_pks[, 1] - MsCoreUtils::ppm(tmp_pks[, 1], 10),
                      tmp_pks[, 1] + MsCoreUtils::ppm(tmp_pks[, 1], 10)),
    rt = tmp_pks[, 2:3])
dr <- file.path(IMAGE_PATH, "alignment_features")
dir.create(dr, recursive = TRUE)

for (i in seq_len(nrow(tmp_pks))) {
    png(paste0(dr, "/EIC_", i, ".png"),
        width = 12, height = 8, units = "cm", res = 600, pointsize = 4)
    eic <- tmp_eics[i]
    plot(eic,
         col = paste0(col_samples, 60),
         peakCol = col_samples[chromPeaks(eic)[, "sample"]],
         peakBg = paste0(col_samples[chromPeaks(eic)[, "sample"]], 10))
    grid()
    legend("topright", col = col_batch,
           legend = names(col_batch), lty = 1)
    legend("topleft",
...
#' What would define/qualify a peak as a good anchor peak?
#' - clear, strong signal
#' - not too broad peak
#' - no other peak left/right of the actual peak
...
- add only oxoproline: rt dev -2 - +2 (same as the good standards to the
...
- unknown003: OK, include as nice peak in batches nr 1,2,3,44,45,46,68,90,91
  (702.49 m/z, 140s)
- EIC_46: OK, include as nice peak in batches nr 1,2,3,44,45,46,68,90,91
  (491.300337 m/z, 120s)

So, decided to add 2 extra unknown but high intensity peak in the rt-range of
120s - 140 rt in the Mix_alignment file.
...
```{r selected_eics_after_alignment, eval = FALSE}
...
```{r EIC-pool-after-alignment, include = FALSE, eval = FALSE}
...
```{r include = FALSE, eval = !file.exists(paste0(RDATA_PATH, "chris_after_alignment.RData"))}
save(chris, file = paste0(RDATA_PATH, "chris_after_alignment.RData"))

```

```{r load_after_refine, include=FALSE, eval = file.exists(paste0(RDATA_PATH, "chris_after_alignment.RData"))}
load(paste0(RDATA_PATH, "chris_after_alignment.RData"))
...
param <- PeakDensityParam(
    ...
    minFraction = 0.5, binSize = 0.01, ppm = 10, bw = 2) #FINDME!!! 0.8

```

```{r correspondence examples examples, eval = FALSE}
...
3-Methylhistidine. #FINDME!!! seperated or need see EIC?

```{r check isomers ok, eval = FALSE}
#' Extract chromatogram with signal for isomers 1-Methylhistidine and
#' 3-Methylhistidine
...
```{r eics_after_correspondence_standards, include = FALSE, eval = FALSE}
...
```{r gap_filling, eval = !file.exists(paste0(RDATA_PATH, "chris_after_gap_filling.RData"))}
...
```{r eval = file.exists(paste0(RDATA_PATH, "chris_after_gap_filling.RData"))}
...
#handle <- file.path(DATA_PATH, 'vmatrix_chris_pos.txt')
#write.table(as.data.frame(featureValues(chris)), file=handle, sep ="\t", row.names = FALSE, col.names = TRUE)

#' write feature information for manual inspection
handle <- file.path(DATA_PATH, 'vm_chris_pos.txt')
write.table(as.data.frame(featureDefinitions(chris)), file=handle, sep ="\t", row.names = FALSE, col.names = TRUE)
...
- NAFLD_Prepro_pos.Rmd
- end-to-end-untargeted-metabolomics.Rmd
