...
# DEFINE PATHS:
RData_PATH <- "/data/massspec/RData"
#RData_PATH <- "Y:/RData" 
...
# Select samples:
fls <- injections[injections$polarity == polarity, ]
fls <- fls[fls$phospholipid_removal == TRUE, ]
fls <- fls[!grepl(paste(exclusion, collapse="|"), fls$mzML_file), ]
fls <- fls$mzML_file
fls <- gsub(".mzML", ".RData", fls)
fls <- fls[!grepl("20161116_POOL_POS_2", fls)]
fls <- fls[!grepl("EQ_MIX", fls)] # remove samples from "equilibration mix"
fls <- fls[!grepl("ACN_IS", fls)] # remove samples from "ACN_IS"
fls <- fls[!grepl("EXTR_IS", fls)] # remove samples from "EXTR_IS"
fls <- fls[!grepl("MS_IS", fls)] # remove samples from "MS_IS"
fls <- fls[!grepl("TEST", fls)] # remove samples from "TEST"
...
```{r alignment-rtime-plot, eval = TRUE, fig.path = IMAGE_PATH, fig.width = 12, fig.height = 10, eval=FALSE}
