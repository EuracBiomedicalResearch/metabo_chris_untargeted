...
if (!file.exists(paste0(RDATA_PATH, "data_neg_ref.RData"))) {
    mnp <- MergeNeighboringPeaksParam(
        ...
        minProp = 0.66)
    ## Peak refinement
    register(MulticoreParam(workers = floor(ncores * 2/3),
                            timeout = 86400L))
    tm <- system.time(
...
bpc_pos <- chromatogram(as(data_pos, "OnDiskMSnExp"), aggregationFun = "max")
...
```{r bpc-neg, eval = !file.exists(paste0(RDATA_PATH, "bpc_neg.RData"))}
bpc_neg <- chromatogram(as(data_neg, "OnDiskMSnExp"), aggregationFun = "max")
...
```{r tics, eval = !file.exists(paste0(RDATA_PATH, "tic_pos.RData"))}
tic_pos <- chromatogram(as(data_pos, "OnDiskMSnExp"), aggregationFun = "sum")
save(tic_pos, file = paste0(RDATA_PATH, "tic_pos.RData"))
tic_neg <- chromatogram(as(data_neg, "OnDiskMSnExp"), aggregationFun = "sum")
